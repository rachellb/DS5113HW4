...
prob = 0.5      # Probability for random walk
...
while done == 0:
    ...
    walk = myPRNG.random()            # Stores random walk prob.
    
    if walk < prob:                     # Checks if random walk is chosen
        ...
        rand_address = myPRNG.randint(0,(len(Neighborhood)-1))      # Generates a random number to pick neighborhood
        rand_neighbor = Neighborhood[rand_address]                  # Grabs a neighborhood from the random address
        x_best = rand_neighbor[:]
        f_best = evaluate(rand_neighbor)[:]
